...
def test():
    ...
    for i in range(num_batch):  # 逐批次处理
        ...
        y_pred_cls[start_id:end_id],y_pred_cls1[start_id:end_id] = session.run([model.y_pred_cls,model.y_pred_cls1], feed_dict=feed_dict)        
...
train()
d,e,label=train1()
f = np.zeros([d.shape[0], 6])
for i in range(d.shape[0]):
    if sum(d[i]) >= 1:
        f[i] = d[i]
    else:
        max1 = max(list(e[i]))
        index1 = list(e[i]).index(max1)
        f[i, index1] = 1

from sklearn.metrics import hamming_loss
from sklearn.metrics import average_precision_score
print('train-score')
print(average_precision_score(label, e))
print(hamming_loss(f, label))
